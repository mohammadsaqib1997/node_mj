<template>
  <div>
    ...
    >
      ...
      <template slot="tbody">
        <tr v-for="(row, ind) in l_data" :key="ind" @click.prevent="nextChild(row.crct_id)">
...
export default {
  ...
  methods: {
    async loadData() {
      ...
      self.loading = true;
      await self.$axios
        .get(`/api/company-hierarchy/crzb-list/1/${self.parent_id}`, {
      ...
      $(".main-content").animate(
        { scrollTop: $('#region-cont').position().top },
        500
      );
      self.loading = false;
    },
