<template lang="pug">
  #navbar.navbar-container
    .wrapper
      ul
        li(v-for="item in menu" :class="{ 'has-dropdown': item.hasOwnProperty('children'), 'is-active': item.active }")
          ...
          template(v-else)
            ...
            template(v-if="item.hasOwnProperty('children')")
              .dropdown
                ul
                  li(v-for="d_child in item.children" :class="{ 'has-popover': d_child.hasOwnProperty('children'), 'is-active': d_child.active }")
                    ...
                    template(v-else)
                      ...
                      template(v-if="d_child.hasOwnProperty('children')")
                        .popover
                          ul
                            li(v-for="p_child in d_child.children" :class="{ 'is-active': p_child.active }")
...
export default {
  ...
  methods: {
    openDropdown: function (e) {
      const prevAct = document.querySelectorAll('div.navbar-container')[0].querySelectorAll('div.wrapper > ul > li.is-u-active')
      if(prevAct.length > 0)
        prevAct[0].classList.remove('is-u-active')
      if(e) {
    ...
    urlActive: function (fullPath) {
      ...
      if(_.has(self.menu[m_ind], 'children')){
        let d_ind = _.findIndex(self.menu[m_ind].children, { name: urlList[1] })
        self.setMItem('['+m_ind+'].children['+d_ind+'].active')

        if(_.has(self.menu[m_ind].children[d_ind], 'children')){
          let p_ind = _.findIndex(self.menu[m_ind].children[d_ind].children, { name: urlList[2] })
          self.setMItem('['+m_ind+'].children['+d_ind+'].children['+p_ind+'].active')
        }
      }
    },
    ...
    resetMenu: function () {
      ...
      if(_.has(self.menu[m_ind], 'children')) {
        ...
        if(_.has(self.menu[m_ind].children[d_ind], 'children')) {
          let p_ind = _.findIndex(self.menu[m_ind].children[d_ind].children, {active: true})
          this.setMItem('['+m_ind+'].children['+d_ind+'].children['+p_ind+'].active', false)
        }
      }
    }
  }
