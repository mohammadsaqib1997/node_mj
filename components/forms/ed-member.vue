<template lang="pug">
    .section
        form#form.form(v-on:submit.prevent="update")
            ...
            b-field(label="ID" :type="(validation.hasError('f_data.id')) ? 'is-danger':''" :message="validation.firstError('f_data.id')")
  ...
  computed: {
    genusername: function() {
      let full_name = this.f_data.full_name
        ...
        .join("");
      let id =
        this.f_data.id.slice(-9) && !isNaN(this.f_data.id.slice(-9))
          ? parseInt(this.f_data.id.slice(-9))
          : "";
      return full_name + id;
    }
  },
    ...
    "f_data.id": {
      ...
      validator: function(value) {
        return Validator.value(value)
          ...
          .custom(() => {
            const self = this;
            if (!Validator.isEmpty(value) && value !== self.edit_id) {
              return self.$axios
